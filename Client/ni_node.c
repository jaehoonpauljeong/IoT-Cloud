...
int main(){
  ...
  int count = 1;
  if(1){
    sprintf(DNSNA, "%s.UID.%s.%s.x.y.COORD.%s.ROOM.%s.BUILDING.%s.LOCd.%s",UNIQUEID, DEVICEMODEL, DEVICECATEGORY, ROOMNUM, BUILDING, LOCATION, DNSSL);
    printf("DNSNA: %s\n", DNSNA);
  }else{
    dnsna_change_flag:
    count++;
    sprintf(DNSNA, "%s.UID.%s.%s%d.x.y.COORD.%s.ROOM.%s.BUILDING.%s.LOCd.%s",UNIQUEID, DEVICEMODEL, DEVICECATEGORY, count, ROOMNUM, BUILDING, LOCATION, DNSSL);
    printf("DNSNA: %s\n", DNSNA);
  }
  ...
  fclose(fptr);
  if(count == 1){
    sprintf(DNSNA_SALA, "%s.UID.%s.%s.%d.%d.COORD.%s.ROOM.%s.BUILDING.%s.LOCs.%s",UNIQUEID, DEVICEMODEL, DEVICECATEGORY, x, y, ROOMNUM, BUILDING, LOCATION, DNSSL);
  }
  else{
    sprintf(DNSNA_SALA, "%s.UID.%s.%s%d.%d.%d.COORD.%s.ROOM.%s.BUILDING.%s.LOCs.%s",UNIQUEID, DEVICEMODEL, DEVICECATEGORY, count, x, y, ROOMNUM, BUILDING, LOCATION, DNSSL);
  }
  printf("DNSNA_SALA: %s\n", DNSNA_SALA);
  char text_buf[256];
  ...
  fclose(fptr);
   ...
   // saving local ip address - Minje -
  inet_ntop(AF_INET6, LOCAL_ADDRESS, debug, SIZE_IPADDRESS_TEXT);
  fptr = fopen("Local_ipaddr.file", "w");
  fprintf(fptr, "%s", debug);
  fclose(fptr);
